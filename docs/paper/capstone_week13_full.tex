...
%  - DONE: Fig. 1 now uses fig1_architectures_week13.png — the redundant top
%    title banner (which still read "Topologies") is cropped, and the three
%    earlier image-content fixes (rung-capability banner, "Deterministic"
%    labels, dropping "Same Roles") were already applied.
%  - The anonymized IEMCON submission is docs/paper/iemcon2026.tex: identical
%    text, anonymous author block, deliberately NOT committed while under
%    double-blind review.
% =============================================================================
...
and under a stronger model is significantly \emph{worse} ($p{=}0.012$), and no
multi-agent workflow exceeds the single-pass baseline's F1---the $3\times$-cost
arms buy recall, the $9\times$ arm buys none. \emph{Verification is:} findings that
...
\midrule
H-specialization \emph{(primary)} & Specialists beat compute-matched generalists & \textbf{Null} ($p{=}0.73$); negative under the Sonnet robustness arm (\S\ref{sec:generation}) \\
...
$|\delta|{\le}0.06$). The Sonnet~4.5 robustness arm---declared in the
pre-registration as a robustness check rather than as a registered
hypothesis---has now run at full scale on the same 99 Qodo PRs, and confirms the
ordering is not a Haiku artifact: Agentless keeps its F1 dominance (0.546 vs.\ 0.388--0.425, Holm
$p{<}0.001$, $\delta$ 0.32--0.45), the recall-not-F1 pattern holds, and H-verify
still fails. H2 fails \emph{harder} rather than identically: the paired median is
again $0.000$, but Hierarchical now trails Generalists-3 on recall significantly
(0.636 vs.\ 0.672, $p{=}0.012$, $\delta{=}{-}0.078$, CI $[-0.143,-0.017]$)---under
the stronger model specialization is not merely no better than compute-matched
sampling but measurably worse. Absolute F1 rises (Agentless $0.487{\to}0.546$)
and the headline ordering is unchanged, but the ordering \emph{within} the
multi-agent arms is not: Consensus and Hierarchical are indistinguishable and
change places (0.425 vs.\ 0.423 under Sonnet; 0.369 vs.\ 0.378 under Haiku). We
therefore claim model-robustness for the registered conclusions and the headline
ordering, not for the ordering among the multi-agent arms.
...
precision- and F1-dominant; the two $3\times$-cost arms buy recall, the
$9\times$ arm does not.
FDR $=1-$precision. Cost/TP $=$ LLM calls per golden-confirmed finding, pooled
over the 80-PR set, counting every finding that matches a golden defect
including repeats---the generous reading for the multi-agent arms, since under
the one-to-one convention used for P/R/F1 the same data gives 0.39/0.94/0.93/3.36
and an $8.6\times$ rather than $5\times$ Consensus premium. P, R, and
F1 are macro means over PRs (each PR the mean of its three runs), so F1 is the
mean of per-PR F1 and is \emph{not} recoverable as the harmonic mean of the P
and R columns---the two differ by 2.0--3.3 points here, as expected when F1 is
averaged rather than derived. Cost/TP is pooled over the registered 80-PR
remainder rather than all 99.}
...
($n{=}79$ PRs with findings; $\bar\Delta{=}{+}0.141$, CI $[0.094,0.189]$,
$p{<}10^{-4}$, $\delta{=}0.364$) at
...
draws saturates by three to four sources (Table~\ref{tab:ladder}), and the union of
...
source worth more than ${\sim}2$ points by leave-one-out---a robust ceiling. The
two numbers measure different axes: Table~\ref{tab:ladder} adds one draw per
\emph{family} and plateaus at 0.66, whereas 0.83 unions every draw, so the
remainder comes from resampling families already in the ladder (the temperature
sweep and the conditioned arm), not from new families---the plateau is in the
family dimension, and the total-draw ceiling sits above it.
...
of Section~\ref{sec:hetero}.) The residue is structured: ${\sim}65\%$
lint-targetable conventions, ${\sim}20\%$ conceptual conventions
(policy/framework rules), and ${\sim}15\%$ a repo-integrated functional
hard-core---36\% of rule violations are reached by \emph{no} source, a
\emph{correlated} blind spot. A deterministic linter out-recalls the eleven-source union
...
saturation-to-a-ceiling phenomenon is not Qodo-specific. Replicating the hetero
ladder on 50 c-CRAB PRs~\cite{crab}---a code-review agent benchmark scored here
against its human review comments, a different oracle from Qodo's injected
defects---reproduces the same shape at a different level
(Table~\ref{tab:ladder}, bottom row): coverage jumps 16 points on the first
added family, plateaus by the second-to-third, and then drifts only from $0.53$
to $0.55$ across the remaining four. Different oracle, different absolute
ceiling, identical plateau.

\begin{table}[t]
\caption{Union recall by number of draws $K$ (semantic matching). Homo $=$ one
model (Haiku) at $T{=}0.7$; hetero $=$ Haiku plus one distinct family per rung
(Kimi, GLM, DeepSeek, Nova, Llama~4, Palmyra). The bottom row repeats the hetero
ladder on a second benchmark with a different oracle.}
\label{tab:ladder}
\centering
\footnotesize
...
$K$ & 1 & 2 & 3 & 4 & 5 & 6 & 7 \\
\midrule
\multicolumn{8}{@{}l}{\emph{Qodo, 97 PRs (injected defects)}}\\
Homo ($T{=}0.7$)  & 0.49 & 0.58 & 0.61 & --- & --- & --- & --- \\
Hetero (families) & 0.51 & 0.61 & 0.64 & 0.65 & 0.65 & 0.66 & 0.66 \\
\addlinespace[2pt]
\multicolumn{8}{@{}l}{\emph{c-CRAB, 50 PRs (human review comments)}}\\
Hetero (families) & 0.36 & 0.52 & 0.53 & 0.53 & 0.53 & 0.55 & 0.55 \\
\bottomrule
\end{tabular}
\\[2pt]{\footnotesize The two Qodo ladders start from different single draws:
homo $K{=}1$ is one Haiku draw at $T{=}0.7$, hetero $K{=}1$ the $T{=}0$
confirmatory anchor---hence 0.49 vs.\ 0.51. The raw hetero$-$homo gap is
${+}0.03$ at $K{=}2,3$, but ${+}0.02$ of it is that anchor offset; net of it the
diversity-attributable increment is ${\approx}{+}0.01$ ($K{=}1{\to}3$ gains of
${+}0.13$ hetero vs.\ ${+}0.12$ homo)---smaller than the raw gap suggests, and
consistent with the family-dimension plateau. Family order affects per-step
marginals, not the $K{=}\max$ total. The c-CRAB ladder plateaus by ${\sim}3$
families, the Qodo hetero ladder by ${\sim}4$; c-CRAB's level is lower because
coverage is scored against sparse human comments.}
\end{table}
...
multi-sampling (0.54 vs.\ 0.57, $p{=}0.44$, $n{=}24$). \emph{Prompt specialization
...
(91\% on known defects); Mistral Large~3 calls 73\% (89\%); DeepSeek~V3.2 calls
only 33\% (71\%).
...
complexity degrades every arm by a similar margin and no arm gains relative
advantage, so no topology rescues complex PRs. The only directional hint runs \emph{against} the
...
\begin{itemize}
  ...
  partly offset this. The Sonnet~4.5 robustness arm has now run at full scale
  ($N{=}99$) and reproduces every registered conclusion
  (Section~\ref{sec:generation}); generalizability of the ordering
  \emph{within} the multi-agent arms remains a limitation rather than a claim,
  since Consensus and Hierarchical swap between the two models. The exploratory
  extensions are small-$n$, single-benchmark probes (execution directional,
  $n{=}6$). Headline
...
compute-matched sampling (the primary null)---and under the stronger model it is
significantly \emph{worse} on recall, so the problem is not that the gain is
small but that the direction is wrong. No arm exceeds the single-pass baseline's
F1; the $3\times$-cost arms buy recall while the $9\times$ arm buys none; the
self-consistency rescue our own pilot suggested does not replicate; and the
effect does not grow with PR complexity. The \emph{verification} axis is
...
arXiv:2404.04834, 2024.

\bibitem{whyfail}
M.~Cemri et al., ``Why do multi-agent LLM systems fail?'' arXiv:2503.13657,
2025.
