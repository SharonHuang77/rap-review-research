...
%  - DONE: Fig. 1 top title banner cropped (fig1_architectures_week13.png);
%    "...Topologies" while the paper now says "Workflows". Crop the top title
%    banner from fig1_architectures_week13.png (or relabel "Topologies" ->
...
confirmatory anchor---hence 0.49 vs.\ 0.51. Diversity bonus (hetero $-$ homo)
$=$ ${+}0.03$ at $K{=}2,3$ on Qodo. Family order affects per-step marginals, not
the $K{=}\max$ total. Both ladders plateau by ${\sim}3$ families; the c-CRAB
level is lower because coverage is scored against sparse human comments.}
\end{table}
