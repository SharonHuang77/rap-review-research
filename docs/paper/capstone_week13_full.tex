...
(agreement or aspect verification) lifts F1 above a plain union.
...
\begin{itemize}
  ...
  partly offset this. The Sonnet~4.5 robustness arm at confirmatory scale is
  deferred (cost prohibitive), so within-multi-agent-arm ranking
  generalizability is a limitation, not a claim; the exploratory extensions are
