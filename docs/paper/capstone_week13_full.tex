...
$9\times$ arm does not. FDR $=1-$precision. P, R, and F1 are macro means over
PRs (each the mean of three runs), so F1 is the mean of per-PR F1, not the
harmonic mean of the columns (the two differ by 2.0--3.3 points). Cost/TP $=$
LLM calls per golden-confirmed finding over the registered 80-PR remainder,
counting repeat reports of one defect separately---unlike the one-to-one
matching behind P/R/F1, which would give an $8.6\times$ Consensus premium
instead of $5\times$.}
