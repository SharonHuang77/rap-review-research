...
as c-CRAB~\cite{crab}. Two exploratory probes sharpen the tooling boundary: a
deterministic checker mirroring the repositories' own lint rules recovers
mechanical conventions the LLM misses (a missing \texttt{Async} suffix at
$43\%$ vs.\ the LLM's $7\%$), so mechanical conventions belong to deterministic
tooling rather than more agents; and supplying whole changed files plus one-hop
dependencies at the review commit does not raise recall over the diff alone
($0.37{\to}0.35$, n.s.), so passive context is not the lever---adaptive,
agentic retrieval and execution is the frontier direction~\cite{swereview}.
