...
def fig2_depth_gradient():
    ...
    # Scope cue: this gradient is measured against injected ground truth; it does
    # NOT extend to the unlabeled industrial setting (Section IV-H). Keep readers
    # from mentally extrapolating it to E3.
    ax.text(0.44, 0.70, "benchmark data only", transform=ax.transAxes,
            ha="center", va="center", fontsize=6.5, style="italic", color="#6B7280",
            bbox=dict(boxstyle="round,pad=0.25", fc="#F3F4F6", ec="#D1D5DB",
                      lw=0.5))
