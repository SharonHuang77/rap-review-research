...
from a production portal (627 reviews)---validates the operational ladder (the
cost and recall orderings hold) but bounds the verification result:
...
leave-one-out cross-family pool coverage for recall). Two operational
patterns transfer cleanly: the communication-cost ladder is unchanged
(Agentless 1~call / 13\,s through Consensus 9~calls / 173\,s), and the recall
ladder reproduces E1's ordering---Agentless has the highest pool coverage
(0.27) and Consensus the lowest (0.10), so extra agents again do not buy
...
\begin{table}[t]
\caption{Industrial arm (30 real PRs): cross-family corroboration-depth
genuine rate. The E1 precision gradient does not replicate under either
genuine-judge.}
