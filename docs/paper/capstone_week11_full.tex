...
agrees at $\kappa{=}0.952$ and reproduces the rows. The signal is carried by the
hard class: of injected functional bugs, 80/61/43\% are caught by ${\geq}1/2/3$
families, versus 45/26/18\% of project-specific rule violations, which a generic
...
for solo findings. This sharpens a pattern already visible on the benchmark,
where three judge families split 33--76\% on whether golden-unmatched findings
were real ($\kappa{=}0.23$--$0.58$): same-issue \emph{matching} is objective and
judge-invariant, while ``is this a real bug?'' is not settled by an LLM judge of
any size.
