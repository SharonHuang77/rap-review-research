...
Table~\ref{tab:depth} makes the mechanism visible: cross-family agreement climbs
steeply with depth where same-model recurrence stays flat before saturating at
its error ceiling. A second run of the same model contributes almost no
independent evidence; a second independent family does. The instrument is
...
\begin{table}[t]
\caption{Corroboration depth: does agreement predict truth? The benchmark
gradient (E1) is steep for independent \emph{families} and flat for repeated runs
of one model. On real unlabeled code (E3) it does not replicate under either
judge. E1 cells are golden-match rate; E3 cells are judge-genuine rate (no oracle
exists). Cluster counts in parentheses.}
\label{tab:depth}
\centering
\footnotesize
\begin{tabular}{@{}lccc@{}}
...
\midrule
E1 same model $\times$3 runs      & 14\% (35)  & 17\% (30)  & 54\% (391) \\
E1 cross-family $\times$3         & 28\% (419) & 51\% (114) & \textbf{89\% (137)} \\
\midrule
E3 industrial, judge Nova Pro     & 100\% (876) & 100\% (23) & 100\% (5) \\
E3 industrial, judge DeepSeek     & 15\% (876)  & 0\% (23)   & 0\% (5) \\
\bottomrule
\end{tabular}
\end{table}
...
This shapes what the trust signal is \emph{for}. The ${\geq}2$-family
``trustworthy'' set decomposes into 52\% functional true positives, 20\% rule
true positives, and 28\% golden-unmatched findings---so cross-family agreement
should be deployed as a high-precision filter for correctness defects, not as a
substitute for project-specific linting, which needs repository context rather
than more reviewers. It also explains why the multi-agent arms gain recall
without gaining quality: the extra findings they surface are concentrated in the
class where models disagree, and disagreement is exactly the regime in which a
finding is unlikely to be real.
...
appear: 21--32\% are localization near-misses---real defects reported at the
wrong line---and high-severity findings are almost always real. The magnitude
awaits human adjudication under a fixed definition; the direction does not. This
audit also predicts what happens next: strip away the golden set entirely, as the
industrial arm does, and the only remaining correctness signal is the one that
...
code (Table~\ref{tab:depth}, lower rows). Cross-family agreement is rare: only 23
...
\textbf{Construct.} E3 lacks ground truth, so its metrics are proxies and are
labelled as such; no correctness is claimed there. Even the objective benchmark
is incomplete---three judge families agree the golden set misses real defects but
disagree on how many (33--76\%)---so golden-referenced precision is a lower
bound. Relative comparisons are unaffected, because incompleteness applies to
every arm identically. The pair judge is validated instrument-side
($\kappa{=}0.95$ over 8{,}061 pairs) but its human validation is pending, so
corroboration numbers remain instrument-relative.
...
further separates structure from raw sampling. The evaluation engine is
deterministic and replays from stored artifacts.
...
stands in its place, and single-model-family generalizability, specifically the
...
registered sample sizes, paired non-parametric tests, effect sizes with CIs, and
Holm correction within families. The finer stratifications and the RQ3 analysis
are exploratory and labeled as such; E3's depth-2/3 counts are small (23 and 5),
so those CIs are wide.
