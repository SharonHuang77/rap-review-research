...
affects review quality, cost, and efficiency. This work-in-progress paper
asks how communication topology affects the effectiveness and efficiency of
automated code review. We compare four approaches under identical
...
judge-invariant, $\kappa{=}0.95$ across two independent pair judges). The industrial arm---30 real merged PRs
from a production portal (627 reviews)---validates the cost/communication ladder and the
more-agents-do-not-help verdict but bounds the verification result:
cross-family agreement is rare on diffuse real findings and the genuineness
judges collapse to $\kappa{=}0.03$, so the precision instrument does not
transfer without a trustworthy correctness oracle. In
