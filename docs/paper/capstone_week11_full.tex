...
(Table~\ref{tab:metrics}). For the industrial case study, which has no
authoritative ground truth, we replace correctness with corroboration
proxies that mirror the E1 cross-family analysis
(Section~\ref{sec:hetero}). \emph{Cross-family corroboration depth} clusters
independent findings with an LLM pair judge (Nova Pro,
$\tau_{\text{pair}}{=}0.7$, union-find); a cluster's depth is the number of
independent model families (Haiku, Kimi, GLM) that produced it, and we report
the fraction of clusters a separate LLM judge rates genuine at each depth.
\emph{Proxy precision} is that per-arm judge-genuine rate, reported under two
independent judges given their disagreement. \emph{Proxy recall} is
leave-one-out cross-family pool coverage---the fraction of the cross-family
pseudo-ground-truth (findings ${\geq}2$ families agree on, rebuilt to exclude
the scored arm's own family) that an arm recovers. Two further signals
specified as triangulation---agreement with conventional static analysis and
later-fix mining of subsequent commits---were not run; the Evidence Score is
retained only as a supporting heuristic, not a correctness measure.
Operational metrics (latency, tokens, cost, LLM calls, messages) are collected
identically across all datasets. Crucially, the industrial corroboration axis
is \emph{cross-family} (independent models), not cross-architecture (one model,
different topologies), since E1 (Section~\ref{sec:hetero}) shows only the former
carries signal.
...
Cross-Family Corroboration Depth & ---      & ---     & \checkmark \\
Proxy Precision (judge-genuine)  & ---      & ---     & \checkmark \\
Proxy Recall (pool coverage)     & ---      & ---     & \checkmark \\
...
industrial-portal arm is reported separately (Section~\ref{sec:industrial})
with corroboration proxies in place of golden labels; its static-analysis and
later-fix producers were not run, so those triangulation signals are omitted
there.
...
premium for a lower-F1 review. The operational ladder itself rises
monotonically in message count, latency, and cost-per-TP (LLM calls tie at
Generalists-3/Hierarchical), confirming H-efficiency. An exploratory Sonnet~4.5 pilot preserves
the \emph{headline} claims---the single-pass F1 lead (0.57 vs.\ 0.39--0.44) and
the H-specialization null both hold---suggesting these are not a Haiku artifact.
It does not preserve the full ordering: the Consensus arm's rank \emph{among the
multi-agent arms} shifted between models. The confirmatory robustness arm is
deferred (Section~\ref{sec:threats}), so the model-family generalizability of
the within-multi-agent ranking remains open.
...
\subsection{RQ3: the topology effect does not grow with PR complexity}
\label{sec:rq3}
The registered stratifier for RQ3 was PR \emph{type} (frontend-only,
backend-only, database-only, cross-component), which requires the diff; the
confirmatory runs did not persist \texttt{rawDiff}, so we substitute the
closest available proxy---the number of distinct files an instance's injected
defects span (median 3, range 1--10)---and label this analysis
\textbf{exploratory} rather than confirmatory. For each registered contrast we
take the per-PR paired difference and test its interaction with complexity both
continuously (Spearman $\rho$) and by median split (Cliff's $\delta$ between
strata).

\textbf{No interaction survives on any contrast} (all Holm $p \ge 0.42$,
$|\delta| \le 0.12$, $|\rho| \le 0.17$). Complexity degrades every arm by a
similar margin---semantic recall falls from 0.52 to 0.46 (Agentless), 0.65 to
0.56 (Generalists-3), 0.65 to 0.56 (Hierarchical), and 0.57 to 0.45
(Consensus)---and the arm ordering is stable across strata, so no topology
rescues complex PRs. The one directional hint runs \emph{against} the
registered prediction: H-communication expected Consensus to beat Hierarchical
especially on cross-component PRs, yet the paired recall difference is 0.000 on
simple PRs and $-0.119$ on complex ones (Holm $p{=}1.0$)---a non-significant
counter-directional trend, not a finding. H-complexity is therefore
\textbf{not supported}.
...
\subsection{Industrial validation: the instrument does not transfer without
an oracle}
\label{sec:industrial}
We ran the registered industrial arm on 30 real merged PRs from the
Indigenomics portal (627 reviews: the four-arm ladder plus the cross-family
agentless axis, three runs each), scored with the same instruments as E1 but
with proxies in place of golden labels (judge-genuine precision;
leave-one-out cross-family pool coverage for recall). Two patterns transfer.
The communication-cost ladder is unchanged (Agentless 1~call / 13\,s through
Consensus 9~calls / 173\,s). And the pool-coverage recall proxy gives the same
qualitative verdict as the benchmarks---more agents do not buy coverage: the
single-pass baseline is highest (0.27) and the most-agentful Consensus lowest
(0.10). (Pool coverage is a different construct from E1's golden recall---on
which Agentless ranked \emph{lowest}---so we compare the ``more agents do not
help'' direction, not the arm ranks.)

The \emph{verification} instrument, however, does not survive contact with
unlabeled code (Table~\ref{tab:industrial}). Cross-family agreement is rare
(only 23 findings reach depth-2 and 5 reach depth-3, of 904 clusters, versus
114 and 137 on E1): diffuse real-world findings do not converge the way
injected defects do. More decisively, the genuineness judges disagree almost
completely here---Nova Pro rates 99\% of findings genuine (a rubber stamp),
DeepSeek V3.2 only 68\%, at $\kappa{=}0.03$---so the
depth$\rightarrow$precision relation is set by judge choice, not
corroboration: flat at 100\% under Nova, and \emph{inverted} under the
discriminating judge (findings families agree on score 0\% genuine, versus
15\% for solo findings). This is the lesson of Section~\ref{sec:completeness}
sharpened: the objective same-issue matcher stays reliable
($\kappa{=}0.95$) while the subjective ``is this genuine?'' judge collapses on
real code. The cross-family precision instrument is thus bounded---it is only
as trustworthy as the correctness oracle behind it, which real unlabeled
repositories do not provide.

\begin{table}[t]
\caption{Industrial arm (30 real PRs): fraction of cross-family clusters
rated real, by corroboration depth. The E1 reference row is \emph{golden-match}
rate (an objective oracle exists); the industrial rows are \emph{judge-genuine}
rate (none does). The E1 precision gradient does not replicate under either
judge.}
...
\midrule
E1 cross-family (reference)  & 28\% & 51\% & \textbf{89\%} \\
Industrial, judge = Nova Pro & 100\% & 100\% & 100\% \\
Industrial, judge = DeepSeek & 15\% & 0\% & 0\% \\
\bottomrule
\end{tabular}
\end{table}
...
reproducibility metadata let us regenerate every metric from saved results.
...
spread of the injected corpus; the industrial arm is now reported (Section~\ref{sec:industrial}), and
execution-grounded
...
exploratory and labeled as such. One registered piece is deferred:
the Sonnet~4.5 robustness arm at confirmatory scale, whose cost is prohibitive
at $100\ \text{PRs} \times 4 \times 3$; the exploratory pilot stands in its
place, and single-model-family generalizability---specifically the ranking
\emph{within} the multi-agent arms---is stated as a limitation rather than
claimed. The industrial-portal arm
(Section~\ref{sec:industrial}) and the RQ3 complexity interaction
(Section~\ref{sec:rq3}, exploratory owing to the stratifier deviation) are now
reported.
...
On real, unlabeled industrial code, however, this instrument reaches its
limit: cross-family agreement is sparse and the LLM genuineness judges
disagree at $\kappa{=}0.03$, so the precision signal transfers only as far as
a trustworthy correctness oracle does (Section~\ref{sec:industrial}).
...
benchmark completeness, and replication on execution-grounded review benchmarks such
as c-CRAB~\cite{crab}.
